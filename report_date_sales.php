...
if(isset($_POST['date']) || isset($_GET['date'])){
   if(isset($_POST['date'])){
		$date = date("Y-m-d", strtotime($_POST['expiry_date']));
	}
	if(isset($_GET['date'])){
		$date = $_GET['date'];   
	}
   ...
   if(isset($order_by)){
		$query = "SELECT * FROM sales WHERE datetime >= '".$date." 00:00:00' AND datetime <= '".$date." 23:49:59' ORDER BY ".$order_by." LIMIT ".($result_list_offset-1).", ".$results_per_page;
   }
}else{
...
<div id="main-copy">
        ...
        <?php if(isset($date)){
            echo "<a href='report_date_sales.php?date=".$date."&order_by=transaction_no DESC'>Transaction ID</a>";
        }?>
        </td>
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_date_sales.php?date=".$date."&order_by=medicine_name ASC'>Medicine</a>";
        }?>
        </td>
        ...
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_date_sales.php?date=".$date."&order_by=datetime DESC'>Date-time</td>";
        }?>
        </td>
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_date_sales.php?date=".$date."&order_by=client_name ASC'>Client</a>";
        }?>
        </td>
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_date_sales.php?date=".$date."&order_by=staff_name ASC'>Staff</a>";
        }?>
        </td>
        </tr>
       <?php
       ...
       <tr>
        ...
        <p align="center"><?php echo " ".($result_list_offset > 1 ? "<a href='report_date_sales.php?increment=-1&result_list_offset=$result_list_offset&date=".$date."&order_by=".$order_by."'><<</a>" : "")." <b>".$range.
        "</b>".($rows == $results_per_page ? "<a href='report_date_sales.php?increment=1&result_list_offset=$result_list_offset&date=".$date."&order_by=".$order_by."'>>></a>" : "")." ";?></p>
        <p><a href="report_date_sales.php?generate_xls=true&date=<?php echo $date;?>&order_by="<?php echo $order_by;?>"">Export to Excel</a></p>
    </div>
        <div id="footer">
         <?php 
		if(isset($connection)){
