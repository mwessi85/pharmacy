...
if (isset($_POST['add'])){
	...
	$today = date("Y-m-d");
	if(isset($_POST['all_service_val'])){
		$service_id = $_POST['all_service_val'];
		$query = "SELECT * FROM services WHERE id = ".$service_id;
		//echo "<p>".$query."</p>";
		$result = mysql_query($query, $connection);
        if(isset($result)){	
		    $services = mysql_fetch_assoc($result);
			$service_id = $services['service'];
			$service = $services['service'];
			$unit_price = $services['unit_cost'];
			$query = "INSERT INTO temp_service_sales(service_name, unit_cost, quantity)
			VALUES ('".$service."', ".$unit_price.", ".$_POST['quantity'].")";
			//echo $query;
			$result = mysql_query($query, $connection);
		}
	}
...
if(isset($_GET['delete_id'])){
	$query_del = "DELETE FROM temp_service_sales WHERE id = ".$_GET['delete_id'];
	$result_del = mysql_query($query_del, $connection);
}
$query = "SELECT * FROM temp_service_sales";
$result = mysql_query($query, $connection);
...
if(isset($_POST['sale'])){
	if($_POST['sale'] == "Lab Test sales >>"){
		header("location: sale_tests.php?customer_val=".$customer_id);	
	}
	$query="SELECT * FROM temp_service_sales";
	...
	if(mysql_num_rows($result_sale)>=1){
		...
		while($sale = mysql_fetch_assoc($result_sale)){
			...
			$query_insert = "INSERT INTO service_sales(transaction_no, service, unit_cost, quantity, datetime, client_id, client_name, staff_id, staff_name)
			VALUES('".$transaction_no."', '".$sale['service_name']."', '".$sale['unit_cost']."', '".$sale['quantity']."', NOW(), '".$customer_id."', '".$customer_name."', '".$_SESSION['user_id']."', '".$staff_name."')";
		...
		$result = mysql_query($query, $connection);
		header("location: after_sale.php?page_reciept=reciept&next=sale_tests&button=Tests&customer_val=".$customer_id."&transaction_no=".$transaction_no);
		}else{
...
<div id="main-copy"> 
        ...
        <h1>Service Sale</h1>
        <form autocomplete="off" enctype="multipart/form-data" action="sale_services.php?transaction_no=<?php echo $transaction_no;?>" method="post">
<table class='results'>
    <tr>
        <td colspan="8" style="background-color:#CCC">
            ...
            <tr>
            <td>Service:<input type="search" name="all_service" id="all_service" value = "" autofocus placeholder="Enter service name" required style="width:400px;"/>
            <input type="hidden" name="all_service_val" id="all_service_val" value = ""/></td>
            <td></td>
            <td>Quantity: <input type="number" name="quantity" id="quantity" maxlength="30" value="" min="1" max="100" placeholder="Quantify the service" required/></td>
		...
		while($temp_sale = mysql_fetch_assoc($result_disp)){
			...
			echo "
		<tr>
		<td ".even_strip($i).">".$temp_sale['service_name']."</td>
		...
		if(isset($temp_sale['id'])){
        	echo "<td ".even_strip($i)."><a href='sale_services.php?delete_id=".$temp_sale['id']."&customer_val=".trim($customer_id)."' onClick='return areYouSure(\"Sure you want to delete this Entry?\")'>Delete</a></td>";
		}
        echo "</tr>";
		$sum = $sum + $total_cost;
		$i++;
        ...
        <form autocomplete="off" enctype="multipart/form-data" id="sale" action="sale_services.php" method="post">
        <?php 
		if($sum != 0){
			if($cash_given < 1){
				$cash_given = 0;
