...
<?php 
   ...
   if(isset($order_by)){
		$query = "SELECT * FROM service_sales WHERE datetime >= '".$date." 00:00:00' AND datetime <= '".$date." 23:49:60' ORDER BY ".$order_by." LIMIT ".($result_list_offset-1).", ".$results_per_page;
   }
?>
<?php include("includes/header.php");?>
<div id="main-copy">
        <h1>Daily Service Sales Report ordered by 
		<?php 
		 if(isset($order_by)){
        ...
        <?php if(isset($date)){
            echo "<a href='report_service_daily_sales.php?date=".$date."&order_by=transaction_no DESC'>Transaction ID</a>";
        }?>
        </td>
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_service_daily_sales.php?date=".$date."&order_by=service ASC'>Service</a>";
        }?>
        </td>
        ...
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_service_daily_sales.php?date=".$date."&order_by=datetime DESC'>Date-time</td>";
        }?>
        </td>
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_service_daily_sales.php?date=".$date."&order_by=client_name ASC'>Client</a>";
        }?>
        </td>
        <td class='column_head'>
        <?php if(isset($date)){
            echo "<a href='report_service_daily_sales.php?date=".$date."&order_by=staff_name ASC'>Staff</a>";
        }?>
        </td>
        </tr>
       <?php
        ...
        <p align="center"><?php echo " ".($result_list_offset > 1 ? "<a href='report_service_daily_sales.php?increment=-1&result_list_offset=$result_list_offset&date=".$date."&order_by=".$order_by."'><<</a>" : "")." <b>".$range.
        "</b>".($rows == $results_per_page ? "<a href='report_service_daily_sales.php?increment=1&result_list_offset=$result_list_offset&date=".$date."&order_by=".$order_by."'>>></a>" : "")." ";?></p>
        <p><a href="report_service_daily_sales.php?generate_xls=true&date=<?php echo $date;?>&order_by="<?php echo $order_by;?>"">Export to Excel</a></p>
    </div>
        <div id="footer">
         <?php 
		if(isset($connection)){
