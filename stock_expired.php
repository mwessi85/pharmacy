...
<div id="main-copy">
        ...
        <td class='column_head'><?php echo "<a href='stock_expired.php?order_by=generic_name ASC'>Medicine</a>";?></td>
        <td class='column_head'><?php echo "<a href='stock_expired.php?order_by=form ASC'>Sales Unit</a>";?></td>
        <td class='column_head'>Stock</td>
        <td class='column_head'>Balance</td>
        <td class='column_head'>Balance(%)</td>  
        <td class='column_head'><?php echo "<a href='stock_expired.php?order_by=expiry_date DESC'>Expiry Date</a>";?></td>
        <td class='column_head'>Buying</td>
        <td class='column_head'>Selling</td>
        <td class='column_head'><?php echo "<a href='stock_expired.php?order_by=stock_date DESC'>Stock Date</a>";?></td>
        <td class='column_head'><?php echo "<a href='stock_expired.php?order_by=staff ASC'>Staff</a>";?></td>
	   ...
	   if($order_by){
		$query = "SELECT m.id medicine_id, m.status medicine_status, m.generic_name generic_name, m.trade_name trade_name, m.weight weight, m.form form, m.unit unit_id, u.value unit, m.category category_id, c.name category,  m.form form_id, f.form medicine_form, f.sales_unit sales_unit, s.id stock_id, s.stock_amount stock_amount, s.current_amount current_amount, s.percentage_balance percentage_balance, s.expiry_date expiry_date, s.buying buying, s.selling selling, s.stock_date stock_date, s.staff staff FROM medicine m JOIN category c JOIN units u JOIN form f JOIN stock s WHERE m.id = s.medicine AND m.category = c.id AND m.unit = u.id  AND m.form = f.id AND s.expiry_date <= NOW() ORDER BY ".$order_by." LIMIT ".($result_list_offset-1).", ".$results_per_page;
	   }
        ...
        if(isset($result)){	
			...
			while($medicine = mysql_fetch_array($result)){
				...
				//$balance = $current_amount/$stock_amount*100;
        ...
        <p align="center"><?php echo " ".($result_list_offset > 1 ? "<a href='stock_expired.php?increment=-1&result_list_offset=$result_list_offset&order_by=".$order_by."'><<</a>" : "")." <b>".$range.
        "</b>".($rows == $results_per_page ? "<a href='stock_expired.php?increment=1&result_list_offset=$result_list_offset&order_by=".$order_by."'>>></a>" : "")." ";?></p>
        <p><a href="add_stock.php">Add Stock</a> | <a href="stock_expired.php?generate_xls=true">Export to Excel</a> | <a href="stock_pending_expiry.php">Stock Pending Expiry</a> | <a href="stock_expired.php">Expired Stock</a></p>
	    </div>
        <div id="footer">
         <?php 
		if(isset($connection)){
