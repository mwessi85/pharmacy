...
if($order_by){
	$query = "SELECT * FROM service_sales ORDER BY ".$order_by." LIMIT ".($result_list_offset-1).", ".$results_per_page;
}
?>
<?php include("includes/header.php");?>
<div id="main-copy">
        <h1>General Service Sales Report ordered by 
		<?php 
		 if(isset($order_by)){
        ...
        <table class='results'>
        <tr>
        <td class='column_head'><?php echo "<a href='report_service_sales.php?order_by=transaction_no DESC'>Transaction ID</a>";?></td>
        <td class='column_head'><?php echo "<a href='report_service_sales.php?order_by=service'>Service</a>";?></td>
        <td class='column_head'>Unit Cost</td>
        <td class='column_head'>Quantity</td>
        <td class='column_head'>Cost</td>
        <td class='column_head'><?php echo "<a href='report_service_sales.php?order_by=datetime DESC'>Date-time</a>";?></td>
        <td class='column_head'><?php echo "<a href='report_service_sales.php?order_by=client_name'>Client</a>";?></td>
        ...
        <p align="center"><?php echo " ".($result_list_offset > 1 ? "<a href='report_service_sales.php?increment=-1&result_list_offset=$result_list_offset&order_by=".$order_by."'><<</a>" : "")." <b>".$range.
        "</b>".($rows == $results_per_page ? "<a href='report_service_sales.php?increment=1&result_list_offset=$result_list_offset&order_by=".$order_by."'>>></a>" : "")." ";?></p>   
        <p><a href="report_service_sales.php?generate_xls=true">Export to Excel</a> | <a href="credit.php">Credit</a></p>
	</div>
        <div id="footer">
         <?php 
		if(isset($connection)){
